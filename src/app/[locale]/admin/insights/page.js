...
import { listArticles } from '@/app/actions/content.js';
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBlockEnd: '1.5rem' }}>
        <h1 className="display d-2" style={{ margin: 0 }}>{t('navInsights')}</h1>
        <Link href="/admin/insights/new" className="btn btn-solid" style={{ fontSize: '.88rem' }}>{t('contentNewInsight')}</Link>
      </div>
      ...
      {articles.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '.6rem' }}>
          {articles.map((a) => {
            const ar = a.article_translations.find((x) => x.locale === 'ar');
            const en = a.article_translations.find((x) => x.locale === 'en');
            const sAr = statusOf(t, ar);
            const sEn = statusOf(t, en);
            return (
              <Link key={a.id} href={`/admin/insights/${a.id}`} style={{
