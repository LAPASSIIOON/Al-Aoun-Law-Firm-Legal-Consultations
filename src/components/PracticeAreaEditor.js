...
import {
  updatePracticeAreaTranslation, approvePracticeAreaTranslation,
  addPracticeAreaTranslation, setPracticeAreaActive,
...
/** @param {{ practiceAreaId: string, translations: any[], myRole: string|null, isActive: boolean }} props */
export default function PracticeAreaEditor({ practiceAreaId, translations, myRole, isActive }) {
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBlockEnd: '1.5rem' }}>
        ...
        <label className="body" style={{ fontSize: '.85rem', display: 'flex', alignItems: 'center', gap: '.5rem', color: 'var(--platinum-2)' }}>
          <input type="checkbox" checked={active} disabled={!canEdit || pending}
            onChange={(e) => { setActive(e.target.checked); startTransition(async () => { await setPracticeAreaActive({ id: practiceAreaId, isActive: e.target.checked }); }); }} />
          {t('contentActive')}
        </label>
      </div>
      ...
      {current ? (
        <ExistingTranslationForm
          key={current.id} tr={current} t={t} canEdit={canEdit} canApprove={canApprove}
          pending={pending} startTransition={startTransition} saved={saved} setSaved={setSaved}
        />
      ) : (
        <NewTranslationForm practiceAreaId={practiceAreaId} locale={tab} t={t} canEdit={canEdit} pending={pending} startTransition={startTransition} />
      )}
    </div>
  );
}

function ExistingTranslationForm({ tr, t, canEdit, canApprove, pending, startTransition, saved, setSaved }) {
  const [title, setTitle] = useState(tr.title);
  const [summary, setSummary] = useState(tr.summary || '');
  ...
  function save() {
    startTransition(async () => {
      await updatePracticeAreaTranslation({ id: tr.id, title, summary, body });
      setSaved(true); setTimeout(() => setSaved(false), 1800);
    });
  }
  function submitForReview() {
    startTransition(async () => {
      await updatePracticeAreaTranslation({ id: tr.id, title, summary, body, status: 'legal_review' });
    });
  }
  function approve() {
    startTransition(async () => { await approvePracticeAreaTranslation(tr.id); });
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <label className="body" style={{ fontSize: '.85rem', color: 'var(--platinum-3)' }}>
        {t('contentSummary')}
        <textarea value={summary} onChange={(e) => setSummary(e.target.value)} disabled={!canEdit} rows={2}
      ...
      <label className="body" style={{ fontSize: '.85rem', color: 'var(--platinum-3)' }}>
        {t('contentBody')}
        <textarea value={body} onChange={(e) => setBody(e.target.value)} disabled={!canEdit} rows={10}
...
function NewTranslationForm({ practiceAreaId, locale, t, canEdit, pending, startTransition }) {
  ...
  function create() {
    ...
    startTransition(async () => {
      const res = await addPracticeAreaTranslation({ practiceAreaId, locale, title, summary, body });
