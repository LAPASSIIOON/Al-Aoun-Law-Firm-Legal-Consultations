...
- **طلبات النماذج العامة لا تُدرج مباشرةً**: الخادم فقط يستدعي دوال الإدراج باستخدام مفتاح الخدمة بعد Turnstile وحدّ المعدل والتحقق. لا تملك أدوار المتصفح تنفيذ هذه الدوال عبر REST.
...
## التحقق الأمني
شغّل `security advisor` بعد كل هجرة. افحص أي تحذير عن دوال `SECURITY DEFINER` أو سياسات RLS قبل اعتماده؛ لا تعتمد على قائمة تحذيرات قديمة.
